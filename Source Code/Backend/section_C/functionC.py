...
def is_valid_roll_no(roll_no):
    """Check if the roll number follows a valid format (only digits or alphanumeric with at least one letter)."""
    return bool(re.match(r'^[A-Za-z]*\d+[A-Za-z]*$', roll_no))  # Must contain at least one digit
...
def findEncoding(imageList, nameList):
    ...
    for img, name in zip(imageList, nameList):
        ...
        if len(encode) > 0:
            ...
            encode.append([0]*128)  # Using 128 zeros to represent an empty encoding
...
def captureImages(name, num_images=15):
    while not is_valid_roll_no(name):
        print("Invalid Roll Number! Please enter a valid format (e.g., A123, 2345, C456).")
        name = input("Enter a valid roll number: ")
    
    directory = 'section_C/database_C'
    ...
    while count < num_images:
        ...
        if key == 32:  # Press SpaceBar to capture
            count += 1
            filename = os.path.join(directory, f'{name}_{count}.jpg')
